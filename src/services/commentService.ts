...
export const getAllCommentsByProjet = async (projetId: number) => {
    return commentRepository.find({ where: { projet: { id: projetId } }, relations: ["author", "projet"] });
}

export const getCommentById = async (id: number) => {
    return commentRepository.findOne({ where: { id }, relations: ["author", "projet"] });
}

export const getAllCommentsByUserId = async (userId: number) => {
    return commentRepository.find({ where: { author: { id: userId } }, relations: ["author", "projet"] });
}

export const getAllRepliesByCommentId = async (commentId: number) => {
    return commentRepository.find({ where: { parentComment: { id: commentId } }, relations: ["author", "projet"] });
}
